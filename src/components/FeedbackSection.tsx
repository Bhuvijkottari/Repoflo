      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center">
          {!showForm ? (
            ...
            >
              <div>
                <label className="font-display font-semibold text-sm text-foreground block mb-2">Rate your experience</label>
                <div className="flex gap-1">
                  {Array.from({ length: 5 }, (_, i) => (
                    <button key={i} type="button" onClick={() => setRating(i + 1)} onMouseEnter={() => setHoverRating(i + 1)} onMouseLeave={() => setHoverRating(0)} className="p-1">
                      <Star className={`w-8 h-8 transition-colors ${i < (hoverRating || rating) ? "fill-primary text-primary" : "text-muted-foreground/30"}`} />
                    </button>
                  ))}
                </div>
              </div>
