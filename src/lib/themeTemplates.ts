...
const wrapTheme = (fontImport: string, fontFamily: string, body: string) =>
  `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">${fontImport}<style>${baseStyle}body{font-family:${fontFamily};line-height:1.7;font-size:16px;overflow-x:hidden}</style></head><body>${body}${scrollScript}</body></html>`;
  ...
  switch (themeId) {
    case "recruiter": {
      ...
      return wrapTheme('', "'Segoe UI',system-ui,sans-serif", `
        <div style="background:#f8fafc;color:#1e293b;min-height:100vh;overflow-x:hidden">
        ...
        <div style="background:#fff;border-bottom:1px solid #e2e8f0;padding:28px 20px"><div style="max-width:900px;margin:0 auto;display:flex;align-items:center;gap:20px;flex-wrap:wrap">
          <div style="width:72px;height:72px;border-radius:50%;overflow:hidden;flex-shrink:0"><img src="${d.avatar}" style="width:100%;height:100%;object-fit:cover"></div>
          <div><h1 style="font-size:1.75rem;font-weight:700">${d.name}</h1><p style="color:#64748b;font-size:1.05rem">${d.title} · ${d.location}</p></div></div></div>
        <div style="max-width:900px;margin:0 auto;padding:28px 20px;overflow-x:hidden">
          <h2 id="overview" style="font-size:1.35rem;font-weight:700;margin:28px 0 14px">GitHub Overview</h2>
          <div class="stats-grid" style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:32px">
            ...
            ].map(([v,l]) => `<div class="reveal" style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:18px;text-align:center;transition:transform .3s ease,box-shadow .3s ease" onmouseover="this.style.transform='translateY(-3px)';this.style.boxShadow='0 8px 24px rgba(0,0,0,.08)'" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='none'"><div style="font-size:1.6rem;font-weight:700;color:#6366f1">${v}</div><div style="font-size:.8rem;color:#64748b;text-transform:uppercase;letter-spacing:1px;margin-top:4px">${l}</div></div>`).join('')}
          </div>
          <h2 id="languages" style="font-size:1.35rem;font-weight:700;margin:28px 0 14px">Top Languages</h2>
          ${s.topLanguages.length ? `<div style="display:flex;border-radius:10px;overflow:hidden;height:28px;margin:8px 0">${s.topLanguages.map((l:any,i:number) => `<div style="width:${l.percentage}%;height:100%;display:flex;align-items:center;justify-content:center;font-size:.75rem;font-weight:600;color:#fff;background:${['#6366f1','#f59e0b','#10b981','#ef4444'][i]}">${l.name} ${l.percentage}%</div>`).join('')}</div>` : '<p style="color:#64748b;font-size:1rem">No language data</p>'}
          ${s.aiGeneratedContent > 0 ? `<div style="background:#fef3c7;border:1px solid #fbbf24;border-radius:10px;padding:14px 18px;margin:14px 0;font-size:1rem;color:#92400e"><strong>AI Generated Content Detected:</strong> ${s.aiGeneratedContent} repositories contain potential AI generated code patterns.</div>` : ''}
          ${s.recentCollaborations.length ? `<h2 style="font-size:1.35rem;font-weight:700;margin:28px 0 14px">Recent Collaborations</h2>${s.recentCollaborations.map((c:string) => `<div class="reveal" style="background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:14px;margin:8px 0;font-size:1rem">${c}</div>`).join('')}` : ''}
          <h2 id="experience" style="font-size:1.35rem;font-weight:700;margin:28px 0 14px">Experience</h2>
          ${d.experience.map(e => `<div class="reveal" style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:22px;margin-bottom:16px;transition:transform .3s ease,box-shadow .3s ease" onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 8px 24px rgba(0,0,0,.06)'" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='none'"><div style="font-weight:600;font-size:1.1rem">${e.role}</div><div style="color:#6366f1;font-size:1rem">${e.company}</div><div style="color:#94a3b8;font-size:.9rem">${e.period}</div><p style="color:#64748b;font-size:1rem;margin-top:6px;line-height:1.7">${e.description}</p></div>`).join('') || '<p style="color:#64748b;font-size:1rem">No experience data. Upload a resume to add this.</p>'}
          <h2 style="font-size:1.35rem;font-weight:700;margin:28px 0 14px">Education</h2>
          ${d.education.map(e => `<div class="reveal" style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:22px;margin-bottom:16px;transition:transform .3s ease,box-shadow .3s ease" onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 8px 24px rgba(0,0,0,.06)'" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='none'"><div style="font-weight:600;font-size:1.1rem">${e.degree}</div><div style="color:#6366f1;font-size:1rem">${e.institution}</div><div style="color:#94a3b8;font-size:.9rem">${e.period}</div></div>`).join('') || '<p style="color:#64748b;font-size:1rem">No education data.</p>'}
          <h2 id="projects" style="font-size:1.35rem;font-weight:700;margin:28px 0 14px">Projects</h2>
          ${d.projects.map(p => `<div class="project-card reveal" style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:22px;margin-bottom:16px"><div style="font-weight:600;font-size:1.1rem">${p.name} ${p.stars ? `<span style="color:#f59e0b">★ ${p.stars}</span>` : ''}</div><p style="color:#64748b;font-size:1rem;margin-top:4px;line-height:1.6">${p.description}</p><div style="margin-top:10px;display:flex;flex-wrap:wrap;gap:6px">${p.tech.map(t => `<span class="skill-tag" style="display:inline-block;background:#f1f5f9;border:1px solid #e2e8f0;padding:6px 12px;border-radius:8px;font-size:.85rem;color:#475569">${t}</span>`).join('')}</div></div>`).join('')}
          <h2 id="skills" style="font-size:1.35rem;font-weight:700;margin:28px 0 14px">Skills</h2>
          <div style="margin-bottom:24px;display:flex;flex-wrap:wrap;gap:6px">${d.skills.map(s => `<span class="skill-tag" style="display:inline-block;background:#f1f5f9;border:1px solid #e2e8f0;padding:6px 14px;border-radius:8px;font-size:.9rem;color:#475569">${s}</span>`).join('')}</div>
        </div></div>`);
    }
    ...
    case "forest":
      return wrapTheme('<link href="https://fonts.googleapis.com/css2?family=Lora:wght@400;500;600;700&display=swap" rel="stylesheet">', "'Lora',serif", `
        ...
        <div style="max-width:760px;margin:0 auto;padding:40px 24px">
          ...
          ${githubActivityHtml(d, '#fff', '#dcfce7', '#16a34a', '#666')}
